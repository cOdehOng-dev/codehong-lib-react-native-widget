...
type Props = NativeStackScreenProps<RootStackParamList, 'TextFieldSample'>;
...
export function TextFieldSampleScreen({ navigation }: Props) {
  ...
  const field1 = new HongTextFieldBuilder()
    .width(300)
  ...
  const field2 = new HongTextFieldBuilder()
    .width(300)
    .padding({ left: 12, top: 10, right: 12, bottom: 10 })
    .fieldRadius({ all: 8 })
    .placeholder('클리어 버튼 있는 텍스트필드')
    .input(text2)
    .clearIcon(HongImages.honglib_ic_20_circle_close_fill)
    .onTextChanged(setText2)
    .applyOption();

  const field3 = new HongTextFieldBuilder()
    .width(300)
    .padding({ left: 12, top: 10, right: 12, bottom: 10 })
    .fieldRadius({ all: 8 })
    .placeholder('비밀번호 입력')
  ...
  const field4 = new HongTextFieldBuilder()
    .width(300)
    .padding({ left: 12, top: 10, right: 12, bottom: 10 })
    .fieldRadius({ all: 8 })
    .placeholder('숫자 입력')
    ...
    .keyboardActionType(HongKeyboardActionType.DONE)
    .onTextChanged(setText4)
    .applyOption();

  const field5 = new HongTextFieldBuilder()
    .width(300)
    .height(100)
    .padding({ left: 12, top: 10, right: 12, bottom: 10 })
    .fieldRadius({ all: 8 })
